...
socketio = SocketIO(
    ...
    manage_session=False  # 禁用Flask-SocketIO会话管理，使用自定义会话管理
)
...
3. 强制规则：
   ...
   - 使用精确匹配，所有术语都要加双引号"""
        ...
        if response.status_code != 200:
            ...
            socketio.emit('search_progress', {
                'stage': 'search_failed',
                'message': f"PubMed搜索请求失败: HTTP {response.status_code}"
            })
        ...
        if not id_list:
            ...
            # 发送尝试更宽泛搜索的信息
            socketio.emit('search_progress', {
                'stage': 'broader_search',
                'message': "未找到文献，尝试更宽泛的搜索策略"
            })
            ...
            if response.status_code == 200:
                ...
                # 发送更宽泛搜索结果信息
                socketio.emit('search_progress', {
                    'stage': 'broader_search_results',
                    'message': f"使用更宽泛策略找到 {total_count} 篇文献",
                    'total_count': total_count
                })
        ...
        socketio.emit('search_progress', {
            ...
            'message': f"搜索完成，成功获取 {len(papers)} 篇文献",
...
def fetch_paper_details(id_list):
    ...
    try:
        ...
        socketio.emit('search_progress', {
            ...
            'total': len(id_list),
            'percentage': 0,
            'message': f"开始获取文献详情，共 {len(id_list)} 篇文献"
        })
        ...
        for i in range(0, len(id_list), batch_size):
            ...
            if response.status_code == 200:
                ...
                socketio.emit('search_progress', {
                    ...
                    'message': f"已获取 {current_count}/{len(id_list)} 篇文献 ({percentage}%)"
                })
            else:
        ...
        # 发送完成信息
        socketio.emit('search_progress', {
            'stage': 'fetch_papers_complete',
            'current': len(all_papers),
            'total': len(id_list),
            'percentage': 100,
            'message': f"文献获取完成，共获取 {len(all_papers)} 篇文献"
        })
    ...
    try:
        ...
        if mode == 'paragraph':
            ...
            for sentence in sentences:
                future = executor.submit(analyze_sentence, sentence, papers_per_sentence, year_start, year_end)
            ...
            for sentence, future in futures:
                try:
                    papers, search_strategy = future.result()
                    if papers:
                        results.append({
                            'text': sentence,
                            'papers': papers,
            ...
            # 生成检索策略
            future = executor.submit(generate_search_strategy, query)
            try:
                search_strategy = future.result()
                return jsonify({
                    'success': True,
                    'search_strategy': search_strategy
                })
            ...
            # 使用线程池执行搜索
            future = executor.submit(search_pubmed, search_strategy)
            try:
                papers, _, total_count, filtered_count = future.result()
                ...
                # 获取文件名（不含路径）
                initial_excel_name = os.path.basename(initial_excel_path)
                initial_word_name = os.path.basename(initial_word_path)
                ...
                # 导出筛选后结果
                filtered_excel_path = export_papers_to_excel(filtered_papers, query, 'filtered')
                filtered_word_path = export_papers_to_word(filtered_papers, query, 'filtered')
                
                filtered_excel_name = os.path.basename(filtered_excel_path)
                filtered_word_name = os.path.basename(filtered_word_path)
                ...
                return jsonify({
                    ...
                    'export_files': {
                        'initial_excel': initial_excel_name,
                        'initial_word': initial_word_name,
                        'filtered_excel': filtered_excel_name,
                        'filtered_word': filtered_word_name
                    }
                })
            except Exception as e:
...
def export_files():
    ...
    try:
        ...
        # 注意：前端已经根据export_type类型传递了正确的papers数据
        # 不需要再次根据is_initial进行筛选，直接使用传递过来的papers
...
def download_file(filename):
    ...
    try:
        ...
        if not re.match(r'^[\w\-. ]+$', filename):
            logger.warning(f"可疑的文件名请求: {filename}")
            return "非法文件名", 400
    ...
    while True:
        try:
            ...
            for file_path in Path(EXPORTS_DIR).glob('*.*'):
                # 如果文件存在超过MAX_CACHE_TIME，则删除
                if now - file_path.stat().st_mtime > MAX_CACHE_TIME:
                    file_path.unlink()
                    count += 1
            if count > 0:
                logger.info(f"已清理 {count} 个过期的导出文件")
        except Exception as e:
...
# 启动清理线程
cleaning_thread = threading.Thread(target=clean_exports_directory, daemon=True)
cleaning_thread.start()

# 添加用户会话和并发处理相关的全局变量
active_users = {}  # 存储活跃用户信息
user_stats = {
    'total_visits': 0,
    'concurrent_users': 0,
    'peak_concurrent_users': 0,
    'visit_times': [],
    'hourly_stats': {str(i): 0 for i in range(24)}  # 初始化24小时统计
}
user_stats_lock = threading.Lock()  # 用于线程安全的统计数据更新
...
performance_stats_lock = threading.Lock()  # 用于性能统计的线程锁

# 线程池执行器
executor = ThreadPoolExecutor(
    max_workers=50,  # 支持最多50个并发任务
    ...
    while True:
        try:
            ...
            with user_stats_lock:
                # 清理超过30分钟不活跃的会话
                inactive_sessions = [
                    sid for sid, data in active_users.items()
                    if (current_time - data['last_active']).total_seconds() > 1800
                ]
                for sid in inactive_sessions:
                    del active_users[sid]
                    update_user_stats(sid, 'disconnect')
                
                # 清理超过24小时的访问记录
                user_stats['visit_times'] = [
                    t for t in user_stats['visit_times']
                ...
                for hour in range(24):
                    if hour != current_time.hour:
                        user_stats['hourly_stats'][str(hour)] = 0
...
def update_user_stats(session_id, action='connect'):
    ...
    Args:
        session_id: 用户会话ID
        action: 动作类型，'connect'表示连接，'disconnect'表示断开
    """
    try:
        with user_stats_lock:
            current_time = datetime.now()
            ...
            if action == 'connect':
                ...
                # 更新小时统计
                if current_hour in user_stats['hourly_stats']:
...
def rate_limit(session_id):
    """简单的速率限制
    
    Returns:
        bool: True表示允许请求，False表示需要限制
    """
    try:
        ...
        # 如果是第一次请求
        if not user_data['last_request_time']:
            user_data['last_request_time'] = current_time
            user_data['request_count'] = 1
            return True
        
        # 计算时间间隔
        time_diff = (current_time - user_data['last_request_time']).total_seconds()
        ...
        # 限制每分钟最多60个请求
        if user_data['request_count'] >= 60:
            return False
        
        user_data['request_count'] += 1
        user_data['last_active'] = current_time  # 更新最后活跃时间
        return True
    except Exception as e:
        logger.error(f"速率限制检查时出错: {str(e)}")
        return False  # 出错时保守处理，返回False
...
def handle_connect():
    ...
    update_user_stats(session_id, 'connect')
    logger.info(f"用户连接 {session_id}, 当前在线人数: {user_stats['concurrent_users']}")

@socketio.on('disconnect')
def handle_disconnect():
    """处理用户断开连接"""
    session_id = request.sid
    update_user_stats(session_id, 'disconnect')
...
def generate_search_strategy(query):
    """生成检索策略
